...
export function Navbar({ isNavbarVisible, opened, toggle, label }) {
    ...
    return (
        <nav className={`w-full flex justify-between items-center px-twoDotFivePercent py-4 sm:px-fivePercent lg:px-tenPercent xl:px-fifteenPercent fixed top-0 left-0 bg-black shadow-[0_12px_20px_rgba(255,255,255,0.1)] transition-opacity duration-300 z-10 ${isNavbarVisible ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
